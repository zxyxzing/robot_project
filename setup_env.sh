...
# 2. Python版本校验（必须为3.10）
log_info "=== 2/5 校验Python3.10环境 ==="
PY_VERSION=$(/usr/bin/python3.10 -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
if [ "$PY_VERSION" != "3.10" ]; then
    log_error "当前Python版本为$PY_VERSION，要求Python3.10，请使用Ubuntu22.04原生系统"
fi
log_info "Python3.10版本校验通过"

# 3. 创建并激活Python3.10虚拟环境（统一命名为 venv）
log_info "=== 3/5 开始创建项目虚拟环境 ==="
python3.10 -m venv venv  # 明确用 python3.10 创建
source venv/bin/activate

# 4. 升级工具并安装项目依赖（分步+镜像源）
log_info "=== 4/5 开始安装项目核心依赖 ==="
python -m pip install --upgrade pip setuptools wheel -i https://pypi.tuna.tsinghua.edu.cn/simple

# 安装 PyTorch（CPU版，若无GPU）
pip install torch==2.0.1+cpu torchvision==0.15.2+cpu torchaudio==2.0.2+cpu --index-url https://download.pytorch.org/whl/cpu -i https://pypi.tuna.tsinghua.edu.cn/simple

# 安装其他依赖
pip install ultralytics==8.0.200 gym==0.26.2 opencv-python numpy pybullet rospy -i https://pypi.tuna.tsinghua.edu.cn/simple

# 5. 安装结果校验
log_info "=== 5/5 开始校验环境安装结果 ==="
if pip check; then
    log_info "✅ 环境搭建完成！无依赖冲突"
    log_info "👉 后续使用请执行以下命令激活虚拟环境：source venv/bin/activate"
else
    log_error "❌ 依赖安装存在冲突，请检查报错信息"
fi
